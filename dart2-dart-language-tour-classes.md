#Classes类
Dart 是一种面向对象语音，包含类和基于 mixin 的继承。每个对象是一个类的实例，并且 Object 是所以类的父类。基于 mixin 的继承指的是除了每个类（除了 Object ）都有一个父类，类体还可以在多个类继承中被复写。
...
在一个实例变量分配一个构造函数参数的模式是很常见的，Dart有语法糖使它容易使用：
...
如果你不声明一个构造函数，系统会提供默认构造函数。默认构造函数没有参数，调用父类的无参数构造函数中。
...
有时一个构造函数的唯一目的是重定向到另一个构造函数在同一个类中。如果一个重载构造函数主体是空的的话，那么调用这个构造函数的时候，调用后面直接加冒号即可。
...
如果你的类产生的对象永远不会改变，你可以把这些对象的编写成常量。为此，定义一个 const 构造函数能确保所有的实例变量是不变的。
...
对象的实例方法可以访问实例变量和 this。以下示例中的 distanceTo() 方法是一个实例方法的一个例子：
...
实现 mixin ，创建一个类继承 Object 类，不声明任何构造函数，
不调用 super 。例如：
